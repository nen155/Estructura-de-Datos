#ifndef __DICCIONARIOV2_HXX
#define __DICCIONARIOV2_HXX
/* 
 * diccionario.h
 * 
...
/** @brief compara dos cadenas tipo string 
@post si es menor el primer parametro devuelve true en caso contrario false
	*/
bool stringCompare( const pair<nombreM,defM> &left, const pair<nombreM,defM> &right ){
   for(pair<nombreM,defM>::const_iterator lit = left.begin(), rit = right.begin(); lit != left.end() && rit != right.end(); ++lit, ++rit )
      if( tolower( (*lit).first ) < tolower( (*rit).first ) )
         return true;
      else if( tolower( (*lit).first  ) > tolower( (*rit).first ) )
         return false;
   return false;
}
...
defM & diccionario::operator[](const nombreM & s) {
	...
	if(i==dic.size()){
		entrada aux2(s,aux);
		dic.push_back(aux2);
		sort(dic.begin(),dic.end(),stringCompare);
	}
	else{
		aux = dic[i].second;
	}
...
bool diccionario::cheq_rep( ) const {
	...
	for(j=0; j < tama && this->dic[j].first.compare(this->dic[j+1].first)<0;j++);

	flag= (k==tama && j==tama  && tama == this->size());
	return flag;
}
